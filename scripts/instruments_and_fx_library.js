...
function SchwaBox(vowelPreset){
	...
	this.filterFreqArray = [
		280, 2550, 2900, // 1, 9.10714286, 10.3571429
		400, 1900, 2550, // 1, 4.75, 6.375
		550, 1770, 2490, // 1, 3.218181818181818, 4.527272727272727
		690, 1660, 2490, // 1, 2.405797101449275, 3.608695652173913
		640, 1190, 2390, // 1, 1.859375, 3.734375
		310, 870, 2250,  // 1, 2.806451612903226, 7.258064516129032
		450, 1030, 2380, // 1, 2.288888888888889, 5.288888888888889
		500, 1500, 2500, // 1, 3, 5
		710, 1100, 2640, // 1, 1.549295774647887, 3.71830985915493
	];
...
MyBuffer.prototype = {
	...
	// multiply buffer values by a ramp with specified start and end points
	// that peaks at a specified value
	makeRamp: function(startPoint, endPoint, upEnd, downStart, upExp, downExp){
		...
		this.rampStart = parseInt(this.buffer.length*this.startPoint);
		this.rampEnd = parseInt(this.buffer.length*this.endPoint);

		this.rampLength = this.rampEnd-this.rampStart;

		this.upLength = parseInt(this.upEnd*this.rampLength);
		this.downLength = parseInt(this.rampLength-(this.rampLength*this.downStart));

		this.upPoint = this.rampStart+this.upLength;
		this.downPoint = this.rampEnd-this.downLength;

		for (this.channel = 0; this.channel<this.buffer.numberOfChannels; this.channel++){
			this.nowBuffering = this.buffer.getChannelData(this.channel);
			for (this.i=0; this.i<this.buffer.length; this.i++){

				if(this.i<this.rampStart){
					this.nowBuffering[this.i] = 0;
				}

				else if(this.i>=this.rampStart && this.i<=this.upPoint){
					this.nowBuffering[this.i] = Math.pow((this.i-this.rampStart)/this.upLength, this.upExp);
				}

				else if(this.i>this.upPoint && this.i<this.downPoint){
					this.nowBuffering[this.i] = 1;
				}

				else if(this.i>=this.downPoint && this.i<this.rampEnd){
					this.nowBuffering[this.i] = Math.pow(1-((this.i-this.downPoint)/this.downLength), this.downExp);
				}

				else if(this.i>=this.rampEnd){
					this.nowBuffering[this.i] = 0;
				}
			}
		}
	...
	// multiply buffer values by a ramp with specified start and end points
	// that peaks at a specified value
	applyRamp: function(startPoint, endPoint, upEnd, downStart, upExp, downExp){
	...
	// add a sine wave of specified frequency and amplitude to the buffer
	addSine: function(freq, amp){
		...
		for (this.channel = 0; this.channel<this.buffer.numberOfChannels; this.channel++){
			...
			for (this.i=0; this.i<this.buffer.length; this.i++){
				this.p = this.i/this.buffer.length;
				this.v = this.a*(Math.sin(this.twoPi*(this.p*this.f)));
				if(Math.abs(this.v) <= 0.00013089969352576765){
					this.nowBuffering[this.i] += 0;
				}
				else if(Math.abs(this.v) > 0.00013089969352576765){
	...
	noiseBand: function(cFreq, bandwidtsh, tuningRange, ampMin, ampMax){

		this.cF = cFreq;
		this.bW = bandwidth;
		this.tuningRange = tuningRange;
		this.aMin = ampMin;
		this.aMax = ampMax;
		this.hB = parseInt(this.bW*0.5);

		this.bandBuffer = new MyBuffer(this.nChannels, this.length, this.sRate);
		...
		for(var i=0; i<this.bW; i++){
			...
			this.f = (this.cF+i)-(this.hB);
			this.a = randomFloat(this.aMin, this.aMax);

			this.bandBuffer.addSine(this.f*randomFloat(this.tuningRange[0], this.tuningRange[1]), this.a);

		}

		// add band to this.buffer
		for(var i=0; i<this.buffer.numberOfChannels; i++){
			this.nowBuffering = this.buffer.getChannelData(i);
			this.newBuffer = this.bandBuffer.buffer.getChannelData(i);
	...
	rampBand: function(cFreq, bandwidth, upHarmonics, midHarmonics, downHarmonics, upTuningRange, midTuningRange, downTuningRange, upEnd, downStart, upExp, downExp, upRange, midRange, downRange){
		...
		this.cF = cFreq;
		this.bW = bandwidth;
		this.upHarmonics = upHarmonics;
		this.midHarmonics = midHarmonics;
		this.downHarmonics = downHarmonics;
		this.upTuningRange = upTuningRange;
		this.midTuningRange = midTuningRange;
		this.downTuningRange = downTuningRange;
		this.upEnd = upEnd;
		this.downStart = downStart;
		this.upExp = upExp;
		this.downExp = downExp;
		...
		this.upPoint = parseInt(this.bW*this.upEnd);
		this.downPoint = parseInt(this.bW*this.downStart);

		this.upLength = this.upPoint;
		this.downLength = this.bW-this.downPoint;

		this.lB = this.cF-this.hB;
		...
		for(var i=0; i<this.bW; i++){
			...
			if(i<=this.upPoint){
				...
				this.a = Math.pow(i/this.upLength, this.upExp)*(randomFloat(this.upRange[0], this.upRange[1]));
				this.bandBuffer.addSine(this.f*randomArrayValue(this.upHarmonics)*randomFloat(this.upTuningRange[0], this.upTuningRange[1]), this.a);

			}

			else if(i>this.upPoint && i<this.downPoint){

				this.a = randomFloat(this.midRange[0], this.midRange[1]);
				this.bandBuffer.addSine(this.f*randomArrayValue(this.midHarmonics)*randomFloat(this.midTuningRange[0], this.midTuningRange[1]), this.a)

			}

			else if(i>=this.downPoint){

				this.a = Math.pow(1-((i-this.downPoint)/this.downLength), this.downExp)*(randomFloat(this.downRange[0], this.downRange[1]));
				this.bandBuffer.addSine(this.f*randomArrayValue(this.downHarmonics)*randomFloat(this.downTuningRange[0], this.downTuningRange[1]), this.a)
	...
	overtones: function(fund, rArray, tuningRange, gArray){
		...
		this.oBuffer = new MyBuffer(this.nChannels, this.length, this.sRate);

		this.f;
		this.a;

		for(var i=0; i<this.rArray.length; i++){

			this.f = this.fund*this.rArray[i]*randomFloat(this.tuningRange[0], this.tuningRange[1]);
			this.a = this.gArray[i];

			this.oBuffer.addSine(this.f, this.a);
		...
		for(var i=0; i<this.buffer.numberOfChannels; i++){
			this.nowBuffering = this.buffer.getChannelData(i);
			this.newBuffer = this.oBuffer.buffer.getChannelData(i);
			for(var j=0; j<this.buffer.length; j++){
				this.nowBuffering[j] += this.newBuffer[j];
			}
		}
...
MyBuffer2.prototype = {
	...
	// output buffer contents at specified time (in seconds)
	startAtTime: function( time , startPoint , duration ){
		...
		this.bufferSource.buffer = this.buffer;
		this.bufferSource.connect(this.output);
		this.bufferSource.start( time , startPoint , duration );
